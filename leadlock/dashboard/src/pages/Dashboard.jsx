...
import {
  Users, CalendarCheck, Timer, Gauge, Activity, TrendingUp
} from 'lucide-react';
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
        ...
        <div className="space-y-0.5 max-h-80 overflow-y-auto">
          ...
          {activity.map((event, i) => (
            <div key={i} className="flex items-start gap-3 text-sm py-2.5 rounded-lg hover:bg-gray-50 px-3 -mx-3 transition-colors group">
              <div className={`w-2 h-2 mt-[7px] rounded-full flex-shrink-0 ${
                ACTIVITY_DOT_COLORS[event.type] || 'bg-gray-300'
              }`} />
              <div className="flex-1 min-w-0">
                <p className="truncate text-gray-600 group-hover:text-gray-900 transition-colors">{event.message}</p>
